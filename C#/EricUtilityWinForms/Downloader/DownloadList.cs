...
namespace EricUtility.Windows.Forms.Downloader
{
    public partial class DownloadList : UserControl
    {
        ...
        {
            DownloadsAction(
                delegate(EricUtilityNetworking.Downloader.Downloader d, ListViewItem item)
                {
                    d.Start();
                }
            );
        ...
        public EricUtilityNetworking.Downloader.Downloader[] SelectedDownloaders
        {
            get
            {
                if (lvwDownloads.SelectedItems.Count > 0)
                {
                    EricUtilityNetworking.Downloader.Downloader[] downloaders = new EricUtilityNetworking.Downloader.Downloader[lvwDownloads.SelectedItems.Count];
                    for (int i = 0; i < downloaders.Length; i++)
                    {
                        downloaders[i] = mapItemToDownload[lvwDownloads.SelectedItems[i]] as EricUtilityNetworking.Downloader.Downloader;
                    }
                    return downloaders;
                }
        ...
        private static string GetResumeStr(EricUtilityNetworking.Downloader.Downloader d)
        {
        ...
        public void UpdateList()
        {
            ...
            for (int i = 0; i < lvwDownloads.Items.Count; i++)
            {
                ...
                if (item == null) return;
                EricUtilityNetworking.Downloader.Downloader d = mapItemToDownload[item] as EricUtilityNetworking.Downloader.Downloader;
                if (d == null) return;
        ...
        private void UpdateSegments()
        {
            try
            {
                ...
                if (lvwDownloads.SelectedItems.Count == 1)
                {
                    ...
                    EricUtilityNetworking.Downloader.Downloader d = mapItemToDownload[newSelection] as EricUtilityNetworking.Downloader.Downloader;
            ...
            if (lvwDownloads.SelectedItems.Count > 0)
            {
                try
                {
                    ...
                    for (int i = lvwDownloads.SelectedItems.Count - 1; i >= 0; i--)
                    {
                        ...
                        action((EricUtilityNetworking.Downloader.Downloader)mapItemToDownload[item], item);
                    }
        ...
        private void UpdateSegmentsInserting(EricUtilityNetworking.Downloader.Downloader d, ListViewItem newSelection)
        {
