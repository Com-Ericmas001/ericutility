...
namespace EricUtilityNetworking.Downloader
{
    public class Downloader
    {
        ...
        private void StartDownloadThreadProc(object objSegmentCount)
        {
            ...
            int segmentCount = (int)objSegmentCount;
            ...
            do
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (currentTry < 10)
                    {
                        ...
                        Thread.Sleep(TimeSpan.FromSeconds(5000));
                    }
                    else
                    {
                        SetState(DownloaderState.NeedToPrepare);
        ...
        private bool RestartFailedSegments()
        {
            ...
            for (int i = 0; i < this.Segments.Count; i++)
            {
                    ...
                    Segments[i].CurrentTry < 10)
                {
                    ...
                    {
                        delay = Math.Max(delay, 5000 - ts.TotalMilliseconds);
                    }
                }
            }
            ...
            lock (segments)
            {
                for (int i = 0; i < this.segments.Count; i++)
                {
                    ...
                    if (oldSegment.State == SegmentState.Downloading &&
                        oldSegment.Left.TotalSeconds > 30 &&
                        oldSegment.MissingTransfer / 2 >= 200000)
                    {
